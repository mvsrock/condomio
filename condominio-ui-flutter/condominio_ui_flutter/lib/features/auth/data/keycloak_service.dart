...
class KeycloakService {
  ...
  ///
  /// - Web: token solo in memoria (nessun persist browser).
  /// - Desktop: ripristina token da secure storage locale.
  /// - Mobile: inizializza wrapper nativo.
  Future<void> init() async {
    if (_initialized) return;

    // Nel flusso manuale:
    // - web: token solo in memoria (hardening XSS)
    // - desktop: token persistiti in secure storage locale
    if (_usesManualFlow) {
      if (_isDesktop) {
        _manualAccessToken = await _readStoredManualToken(
        ...
        // Cleanup difensivo per rimuovere eventuali token legacy salvati
  ...
  /// Ripristina token da storage locale (solo desktop).
  ...
  Future<void> _persistManualTokens() async {
    // Persistenza token solo desktop.
  ...
  Future<String?> _readStoredManualToken(String key) async {
    if (kIsWeb) {
      // Web hardening: nessuna persistenza token su storage browser.
      return null;
    }
